...
class splitting_type(IntEnum):
    """Enumeration of splitting types used in X-Means."""
    BAYESIAN_INFORMATION_CRITERION = 0
    MINIMUM_NOISELESS_DESCRIPTION_LENGTH = 1


class xmeans:
    """X-Means clustering algorithm class."""

    def __init__(self, data, initial_centers=None, kmax=20, tolerance=0.001,
    ...
    def __search_optimial_parameters(self, local_data):
        ...
        for _ in range(self.__repeat):
            ...
            local_centers = kmeans_plusplus_initializer(local_data, 2, candidates, random_state=42).initialize()
    ...
    def __bayesian_information_criterion(self, clusters, centers):
        ...
        if N - K > 0:
            sigma_sqrt /= (N - K)
            p = (K - 1) + dimension * K + 1
            sigma_multiplier = float('-inf') if sigma_sqrt <= 0.0 else dimension * 0.5 * log(sigma_sqrt)
    ...
    def __verify_arguments(self):
        ...
        if self.__tolerance < 0:
            raise ValueError("Tolerance (current value: '%d') should be greater or equal to 0." % self.__tolerance)

        if self.__repeat <= 0:
            raise ValueError("Repeat (current value: '%d') should be greater than 0." % self.__repeat)

        if self.__alpha < 0.0 or self.__alpha > 1.0:
            raise ValueError("Parameter for the probabilistic bound Q(alpha) should be in the range [0, 1] (current: '%f')." % self.__alpha)

        if self.__beta < 0.0 or self.__beta > 1.0:
            raise ValueError("Parameter for the probabilistic bound Q(beta) should be in the range [0, 1] (current: '%f')." % self.__beta)
